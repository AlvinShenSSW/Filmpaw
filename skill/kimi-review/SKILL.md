---
name: kimi-review
description: Run Kimi (Kimi Code CLI) as the default final, read-only, second-opinion review gate on the current PR/branch, then triage and fix the findings. Kimi is the default 终审 (final external review, runs LAST after the 外门) in both AFK variants — the 外门 is Codex under default /afk and Claude under /afk codex; Kilo/GLM 5.2 may replace Kimi only when the operator explicitly selects an experimental Kilo final gate; never run Codex as a reviewer under /afk codex. Interchangeable with codex-review interactively when reviewer != implementer. Scoped to architecture + real bugs. CTO review first, external review last. Triggers include "/kimi-review", "run kimi review", "kimi gate".
...
handed back for approval by default. Kimi is **the default 终审 (final external review), run last
after the 外门**, in both AFK variants: the 外门 is the
[Codex review](../codex-review/SKILL.md) under default `/afk` and Claude under
`/afk codex` (both stages run). Optional MiMo/Kilo add-ons may run between the 外门
and Kimi. Kilo can replace Kimi only when the operator explicitly chooses an
experimental Kilo/GLM 5.2 final gate. If Kimi self-skips, the run leans on the 外门 alone

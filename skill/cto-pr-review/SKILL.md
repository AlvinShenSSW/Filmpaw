...
This project pairs the CTO review with **two required independent external reviews**
as the final checks *after* the CTO verdict. Sequence: **CTO first, then 外门,
optional MiMo/Kilo add-on, then Kimi 终审 last by default.** Both required external reviews run, and
each must be a different model from the implementer (and from each other):
...
- **MiMo (`/mimo-review`, `mimo/mimo-v2.5-pro`)** may run as an optional add-on or
  backup after the 外门 and before Kimi. It never replaces Kimi as final gate.
- **Kilo (`/kilo-review`, `z-ai/glm-5.2`)** may run as an optional add-on or
  backup after the 外门 and before Kimi. It may be an experimental final gate only
  when the operator explicitly selects Kilo/GLM 5.2 终审; record that choice.
...
- **You are not the last gate.** Two required **external reviews** run after your verdict in `afk` mode (operator's call interactively): the **外门** then the **Kimi 终审** by default, both run. The 外门 is the non-implementer driver model — **Codex** under default `/afk`, **Claude** under `/afk codex` (never Codex there); **Kimi is the default 终审**. MiMo/Kilo may be inserted as optional add-ons before Kimi. Kilo may be an experimental 终审 only when the operator explicitly selects it. Keep external invocations to a minimum: batch all structural fixes + a self-review pass between runs, and defer documentation/minor items to one final pass.
